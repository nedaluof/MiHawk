...
class MiAesCipherManagerImpl(
  ...
  override fun isInitialized(): Boolean = isSupported
   ...
   */
  override fun encrypt(plainText: String, key: String): MiEncryptionSpec {
    if (!isSupported) {
   ...
   */
  override fun decrypt(spec: MiEncryptionSpec): String {
    if (!isSupported) {
